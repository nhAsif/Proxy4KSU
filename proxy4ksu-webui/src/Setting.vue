<template>
    <van-pull-refresh class="cyber-page cyber-settings" v-model="loading" @refresh="onRefresh" :pulling-text="t('common.pulling-text')"
                      :loosing-text="t('common.loosing-text')" :loading-text="t('common.loading-text')">
        <section class="cyber-hero" aria-labelledby="settings-title">
            <div class="cyber-kicker">{{ config.xrayHelper.coreType || 'core' }} config</div>
            <h1 id="settings-title" class="cyber-title">{{ t('common.setting') }}</h1>
            <p class="cyber-subtitle">{{ config.proxy.method || 'proxy' }} / {{ config.xrayHelper.dataDir || '/data/adb/xray' }}</p>
        </section>
