<template>
    <van-pull-refresh class="cyber-page cyber-dashboard" v-model="loading" :disabled="disablePull" @refresh="onRefresh" :pulling-text="t('common.pulling-text')"
                      :loosing-text="t('common.loosing-text')" :loading-text="t('common.loading-text')">
        <section class="cyber-hero" aria-labelledby="dashboard-title">
            <div class="cyber-kicker">{{ status.coreType || 'xray' }} uplink</div>
            <h1 id="dashboard-title" class="cyber-title">{{ t('common.module-name') }}</h1>
            <p class="cyber-subtitle">{{ coreStatus }} / {{ status.method || 'proxy' }} / pid {{ status.pid || 'offline' }}</p>
        </section>
        <!-- cell-group version -->
        <van-cell-group :title="t('dashboard.version')" inset>
            <van-cell :title="t('dashboard.version-module')" title-style="max-width:35%;" :value="version"
                      url="https://github.com/nhAsif/Proxy4KSU" clickable/>
            <van-cell :title="t('dashboard.version-dashboard')" title-style="max-width:35%;" :value="t('common.dashboard-version')"
                      url="https://github.com/nhAsif/Proxy4KSU/tree/master/proxy4ksu-webui" clickable/>
        </van-cell-group>
        <!-- cell-group status -->
...
defineProps(["theme"]);
const MODULE_PROP = "/data/adb/modules/proxy4ksu/module.prop"
